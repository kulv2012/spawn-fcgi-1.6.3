...
int main(int argc, char **argv) {
	...
	while (-1 != (o = getopt(argc, argv, "c:d:f:g:?hna:p:u:vC:F:s:P:U:G:M:S"))) {
		switch(o) {
		case 'f': fcgi_app = optarg; break;//FCGI可执行文件地址
	...
	if (optind < argc) {//额外的参数给fcgi进程当参数
	...
	if (pid_file &&
        ...
        //正常也不会使用PID file的,如果带了-P选项，就会将创建的子进程PID写到这个文件，每行一个
		struct stat st;
		if (errno != EEXIST) {
	...
	if (i_am_root) {// 一般都不是root了的
		uid_t uid, sockuid;
		gid_t gid, sockgid;
		const char* real_username;
        //如果是root,就需要找一个合适的用户来创建进程
